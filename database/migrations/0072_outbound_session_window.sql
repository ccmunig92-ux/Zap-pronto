...
-- Free-form WhatsApp messages are legal only while the customer service
-- window is open.  The worker must receive this decision from the database,
-- where the tenant-scoped inbound history is authoritative; it must not infer
-- it from process-local state or from an arbitrary message timestamp.
...
RETURNS TABLE(tenant_id uuid,outbox_id uuid,message_id uuid,channel_connection_id uuid,
  channel_account_id text,secret_reference text,recipient_external_id text,body text,
  session_open boolean,event_type text,payload_version integer,lease_token uuid)

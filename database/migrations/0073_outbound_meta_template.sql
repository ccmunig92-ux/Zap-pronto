...
-- Approved WhatsApp templates are represented in the message payload by the
-- delivery producer.  This migration only transports the non-secret
-- template contract to the worker; approval and credentials remain external
-- Meta configuration.  Null values preserve the existing text contract.
DROP FUNCTION IF EXISTS public.claim_outbound_delivery_events(integer,integer);
...
RETURNS TABLE(tenant_id uuid,outbox_id uuid,message_id uuid,channel_connection_id uuid,
  channel_account_id text,secret_reference text,recipient_external_id text,body text,session_open boolean,
  template_name text,template_language_code text,template_components jsonb,
  event_type text,payload_version integer,lease_token uuid)
...
BEGIN
  ...
  WITH candidates AS (
    ...
    JOIN public.channel_connections connection ON connection.tenant_id=conversation.tenant_id
      AND connection.id=conversation.channel_connection_id AND connection.type='WHATSAPP' AND connection.status='ACTIVE'
      AND connection.secret_reference IS NOT NULL AND pg_catalog.btrim(connection.secret_reference) <> ''
  ...
  SELECT leased.tenant_id,leased.id,message.id,conversation.channel_connection_id,
    connection.external_account_id,connection.secret_reference,identity.external_user_id,message.body,
    EXISTS (SELECT 1 FROM public.messages inbound
      WHERE inbound.tenant_id=message.tenant_id AND inbound.conversation_id=message.conversation_id
        AND inbound.direction='INBOUND' AND inbound.created_at > pg_catalog.clock_timestamp()-interval '24 hours') AS session_open,
    NULLIF(message.payload->'meta_template'->>'name',''),
    NULLIF(message.payload->'meta_template'->>'language_code',''),
    CASE WHEN jsonb_typeof(message.payload->'meta_template'->'components')='array'
      THEN message.payload->'meta_template'->'components' ELSE NULL END,
    leased.event_type,leased.payload_version,leased.lease_token

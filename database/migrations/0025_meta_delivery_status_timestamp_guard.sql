...
ALTER TABLE meta_delivery_status_applications
  DROP CONSTRAINT meta_delivery_status_applications_outcome_check;
ALTER TABLE meta_delivery_status_applications
  ADD CONSTRAINT meta_delivery_status_applications_outcome_check CHECK(outcome IN(
    'APPLIED','IGNORED_DUPLICATE','IGNORED_STALE','IGNORED_CANCELLED','IGNORED_INVALID_TIMESTAMP',
    'UNMATCHED','AMBIGUOUS','UNSUPPORTED','RECIPIENT_MISMATCH'));

ALTER FUNCTION reconcile_meta_delivery_status(text,text,text,text,text,timestamptz,integer[],text,text,text)
  RENAME TO reconcile_meta_delivery_status_0024;
REVOKE ALL ON FUNCTION reconcile_meta_delivery_status_0024(text,text,text,text,text,timestamptz,integer[],text,text,text)
...
DECLARE connection_record record;existing_receipt public.meta_delivery_status_receipts%ROWTYPE;
  existing_application public.meta_delivery_status_applications%ROWTYPE;created_receipt_id uuid;
  created_application_id uuid;candidate_total integer:=0;
BEGIN
  ...
  IF FOUND THEN
    ...
    RETURN NEXT;RETURN;
  END IF;
  IF requested_occurred_at>clock_timestamp()+interval '10 minutes' THEN
    ...
    VALUES(connection_record.tenant_id,created_receipt_id,NULL,'IGNORED_INVALID_TIMESTAMP',NULL,NULL,candidate_total)
    ...
    VALUES(connection_record.tenant_id,'SYSTEM',NULL,'META_DELIVERY_STATUS_RECONCILED','meta_delivery_status_receipt',created_receipt_id::text,
      jsonb_build_object('receiptId',created_receipt_id,'applicationId',created_application_id,
        'messageId',NULL,'outcome','IGNORED_INVALID_TIMESTAMP','candidateCount',candidate_total));
    receipt_id:=created_receipt_id;application_id:=created_application_id;message_id:=NULL;
    outcome:='IGNORED_INVALID_TIMESTAMP';previous_status:=NULL;result_status:=NULL;
    candidate_count:=candidate_total;replayed:=false;RETURN NEXT;RETURN;
  END IF;
  ...
  FROM public.reconcile_meta_delivery_status_0024(requested_channel_account_id,requested_external_message_id,

...
-- Do not lease an event unless all delivery joins are already valid.  The
-- previous function leased first and joined later, which could leave an event
-- PROCESSING while returning no row when a connection or identity was stale.

...
ALTER TABLE handoff_transfer_commands ADD COLUMN reason text;
UPDATE handoff_transfer_commands SET reason='LEGACY_UNSPECIFIED' WHERE reason IS NULL;
ALTER TABLE handoff_transfer_commands ALTER COLUMN reason SET NOT NULL;
ALTER TABLE handoff_transfer_commands ADD CONSTRAINT handoff_transfer_commands_reason_check
  CHECK(reason IN('LEGACY_UNSPECIFIED','SHIFT_CHANGE','LOAD_BALANCING','SPECIALIZED_SUPPORT','OPERATIONAL_CONTINUITY'));
...
DECLARE normalized_key text;normalized_reason text;computed_fingerprint text;legacy_fingerprint text;
  command_record public.handoff_transfer_commands%ROWTYPE;handoff_record public.human_handoffs%ROWTYPE;
  conversation_record public.conversations%ROWTYPE;case_record public.service_cases%ROWTYPE;target_record uuid;
  next_handoff integer;next_conversation integer;now_at timestamptz:=now();
BEGIN
  PERFORM public.assert_app_context_authorized();
  normalized_key:=btrim(requested_key); normalized_reason:=btrim(requested_reason);
  IF requested_handoff_id IS NULL OR requested_target_user_id IS NULL
    ...
    OR normalized_reason NOT IN('SHIFT_CHANGE','LOAD_BALANCING','SPECIALIZED_SUPPORT','OPERATIONAL_CONTINUITY') THEN
    RAISE EXCEPTION 'INVALID_HANDOFF_TRANSFER_REQUEST' USING ERRCODE='P0001';
  END IF;
  computed_fingerprint:=encode(digest(convert_to(format(
    '{"expectedVersion":%s,"handoffId":"%s","reason":"%s","targetUserId":"%s"}',requested_expected_version,
    lower(requested_handoff_id::text),normalized_reason,lower(requested_target_user_id::text)),'UTF8'),'sha256'),'hex');
  legacy_fingerprint:=encode(digest(convert_to(format('{"expectedVersion":%s,"handoffId":"%s","targetUserId":"%s"}',
  ...
  IF FOUND THEN
    IF command_record.handoff_id<>requested_handoff_id OR command_record.expected_version<>requested_expected_version
      ...
      OR (command_record.reason='LEGACY_UNSPECIFIED' AND command_record.request_fingerprint<>legacy_fingerprint)
      OR (command_record.reason<>'LEGACY_UNSPECIFIED' AND (command_record.reason<>normalized_reason
  ...
  INSERT INTO public.workflow_transitions(tenant_id,aggregate_type,aggregate_id,from_status,to_status,reason,actor_id,correlation_id,metadata) VALUES
    (handoff_record.tenant_id,'HANDOFF',handoff_record.id,'ACTIVE','ACTIVE','ATTENDANT_TRANSFERRED',public.current_app_actor_id(),current_setting('app.correlation_id'),jsonb_build_object('fromUserId',public.current_app_actor_id(),'toUserId',requested_target_user_id,'reason',normalized_reason)),
    (handoff_record.tenant_id,'CONVERSATION',conversation_record.id,'HUMAN_ACTIVE','HUMAN_ACTIVE','ATTENDANT_TRANSFERRED',public.current_app_actor_id(),current_setting('app.correlation_id'),jsonb_build_object('fromUserId',public.current_app_actor_id(),'toUserId',requested_target_user_id,'reason',normalized_reason));
  INSERT INTO public.audit_events(tenant_id,actor_type,actor_id,action,entity_type,entity_id,metadata)
    VALUES(handoff_record.tenant_id,'USER',public.current_app_actor_id(),'HANDOFF_TRANSFERRED','handoff',handoff_record.id::text,jsonb_build_object('fromUserId',public.current_app_actor_id(),'toUserId',requested_target_user_id,'reason',normalized_reason,'handoffVersion',next_handoff));
  INSERT INTO public.outbox_events(tenant_id,aggregate_type,aggregate_id,event_type,payload,idempotency_key)
    VALUES(handoff_record.tenant_id,'handoff',handoff_record.id,'handoff.transferred',jsonb_build_object('handoffId',handoff_record.id,'conversationId',conversation_record.id,'fromUserId',public.current_app_actor_id(),'toUserId',requested_target_user_id,'reason',normalized_reason,'handoffVersion',next_handoff,'conversationVersion',next_conversation),'handoff.transferred:'||handoff_record.id::text||':'||next_handoff::text);
  INSERT INTO public.handoff_transfer_commands(tenant_id,idempotency_key,handoff_id,expected_version,target_user_id,actor_id,
    request_fingerprint,conversation_id,service_case_id,handoff_version,conversation_version,correlation_id,unit_id,reason)
  VALUES(handoff_record.tenant_id,normalized_key,handoff_record.id,requested_expected_version,requested_target_user_id,
    public.current_app_actor_id(),computed_fingerprint,conversation_record.id,case_record.id,next_handoff,next_conversation,
    current_setting('app.correlation_id'),handoff_record.unit_id,normalized_reason);
...
RETURNS uuid LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
  SELECT unit_id FROM (
    ...
    WHERE command.tenant_id=public.current_app_tenant_id() AND command.idempotency_key=btrim(requested_key)
      ...
      AND ((command.reason='LEGACY_UNSPECIFIED' AND command.request_fingerprint=encode(digest(convert_to(format(
        '{"expectedVersion":%s,"handoffId":"%s","targetUserId":"%s"}',requested_expected_version,
        lower(requested_handoff_id::text),lower(requested_target_user_id::text)),'UTF8'),'sha256'),'hex'))
        OR (command.reason=btrim(requested_reason) AND command.request_fingerprint=requested_fingerprint))
  ) authorized ORDER BY precedence LIMIT 1
$$;
REVOKE ALL ON FUNCTION resolve_inbox_handoff_transfer_unit(uuid,integer,uuid,text,text)
  FROM PUBLIC,zap_pronto_app,zap_pronto_api,zap_pronto_worker;

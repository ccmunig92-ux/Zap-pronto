...
-- The worker receives only an opaque pointer to the server-side secret.  The
-- pointer is tenant/connection scoped; the secret material never crosses the
-- database boundary and is never returned by this function.
...
RETURNS TABLE(tenant_id uuid,outbox_id uuid,message_id uuid,channel_connection_id uuid,
  channel_account_id text,secret_reference text,recipient_external_id text,body text,event_type text,payload_version integer,
  lease_token uuid)
...
BEGIN
  ...
  SELECT leased.tenant_id,leased.id,message.id,conversation.channel_connection_id,
    connection.external_account_id,connection.secret_reference,identity.external_user_id,message.body,leased.event_type,

...
INSERT INTO users(id,tenant_id,email,display_name)VALUES
 ('99000000-0000-4000-8000-000000000006','99000000-0000-4000-8000-000000000001','manager-replay-assignment@test.local','Replay manager');
INSERT INTO user_units(tenant_id,user_id,unit_id,role)VALUES
 ('99000000-0000-4000-8000-000000000001','99000000-0000-4000-8000-000000000006','99000000-0000-4000-8000-000000000002','UNIT_MANAGER');
SELECT set_config('app.actor_id','99000000-0000-4000-8000-000000000006',true);
DO $$DECLARE fp text;BEGIN
 fp:=encode(digest(convert_to('{"unitId":"99000000-0000-4000-8000-000000000002","mode":"ENFORCE_NEW_ASSIGNMENTS","expectedVersion":2}','UTF8'),'sha256'),'hex');
 BEGIN PERFORM set_unit_assignment_policy('99000000-0000-4000-8000-000000000002','ENFORCE_NEW_ASSIGNMENTS',2,'assignment-policy-key-2',fp);
  RAISE EXCEPTION 'CROSS_ACTOR_POLICY_REPLAY_ACCEPTED';EXCEPTION WHEN SQLSTATE 'P0001' THEN
  IF SQLERRM<>'ASSIGNMENT_POLICY_IDEMPOTENCY_CONFLICT' THEN RAISE;END IF;END;
END$$;

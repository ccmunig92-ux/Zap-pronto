...
DO $$ BEGIN IF has_table_privilege('zap_pronto_api','unit_capacity_alert_policy_versions','SELECT') OR has_function_privilege('zap_pronto_worker','get_unit_capacity_alert_snapshot(uuid,timestamptz)','EXECUTE') OR NOT has_function_privilege('zap_pronto_api','get_unit_capacity_alert_snapshot(uuid,timestamptz)','EXECUTE') THEN RAISE EXCEPTION 'CAPACITY_ALERT_PRIVILEGE_INVALID';END IF;IF NOT(SELECT relrowsecurity AND relforcerowsecurity FROM pg_class WHERE oid='unit_capacity_alert_policy_versions'::regclass)THEN RAISE EXCEPTION 'CAPACITY_ALERT_RLS_NOT_FORCED';END IF;END$$;
ROLLBACK;

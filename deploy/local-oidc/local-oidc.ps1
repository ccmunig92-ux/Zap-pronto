...
function E2E {
  ...
  try{foreach($entry in $vars.GetEnumerator()){Set-Item "Env:$($entry.Key)" $entry.Value};Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor reconhece alerta de SLA uma única vez')
    $slaAlertState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM handoff_sla_acknowledgements WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060' AND acknowledged_by_user_id='90000000-0000-4000-8000-000000000012' AND handoff_version=1 AND version=1; SELECT count(*) FROM handoff_sla_acknowledge_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060' AND actor_id='90000000-0000-4000-8000-000000000012' AND expected_version=1; SELECT count(*) FROM audit_events WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND action='SLA_ALERT_ACKNOWLEDGED' AND entity_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($slaAlertState|Select-Object -First 3|Where-Object{$_-ne'1'}).Count-ne 0-or @($slaAlertState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_SLA_ALERT_ACKNOWLEDGEMENT_STATE_INVALID'}

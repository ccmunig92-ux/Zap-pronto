...
function Reconcile-SyntheticSupervisor($Values) {
  $stage='AUTHENTICATE'
  $base=@('compose','--project-name',$projectName,'--env-file',$envFile)+$composeFiles+@('exec','-T','keycloak','/opt/keycloak/bin/kcadm.sh')
  try {
    & docker @base config credentials --server 'http://127.0.0.1:8080' --realm master --user $Values.LOCAL_KEYCLOAK_ADMIN_USERNAME --password $Values.LOCAL_KEYCLOAK_ADMIN_PASSWORD *> $null
    if($LASTEXITCODE-ne 0){throw 'AUTHENTICATION_FAILED'}
    $stage='LOOKUP'
    $raw=(& docker @base get users -r zap-pronto-local -q username=supervisor.local -q exact=true --fields id,username|Out-String)
    if($LASTEXITCODE-ne 0){throw 'LOOKUP_FAILED'};$users=@($raw|ConvertFrom-Json)
    if($users.Count-eq 0){
      $stage='CREATE'
      & docker @base create users -r zap-pronto-local -s id=91000000-0000-4000-8000-000000000004 -s username=supervisor.local -s email=supervisor.local@example.test -s emailVerified=true -s enabled=true -s firstName=Supervisor -s lastName=Local *> $null
      if($LASTEXITCODE-ne 0){throw 'CREATE_FAILED'}
      $stage='LOOKUP_CREATED'
      $raw=(& docker @base get users -r zap-pronto-local -q username=supervisor.local -q exact=true --fields id,username|Out-String)
      if($LASTEXITCODE-ne 0){throw 'LOOKUP_CREATED_FAILED'};$users=@($raw|ConvertFrom-Json)
    }
    if($users.Count-ne 1-or$users[0].id-notmatch'^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'){throw 'IDENTITY_MISMATCH'}
    $userId=$users[0].id
    $stage='UPDATE'
    & docker @base update "users/$userId" -r zap-pronto-local -s username=supervisor.local -s email=supervisor.local@example.test -s emailVerified=true -s enabled=true -s firstName=Supervisor -s lastName=Local *> $null
    if($LASTEXITCODE-ne 0){throw 'UPDATE_FAILED'}
    $stage='PASSWORD'
    & docker @base set-password -r zap-pronto-local --userid $userId --new-password $Values.LOCAL_OIDC_SUPERVISOR_PASSWORD --temporary=false *> $null
    if($LASTEXITCODE-ne 0){throw 'PASSWORD_FAILED'}
    $userId
  }catch{throw "LOCAL_OIDC_SYNTHETIC_SUPERVISOR_RECONCILIATION_FAILED:$stage"}
}
function Sync-SyntheticSupervisorSubject([string]$Subject) {
  if($Subject-notmatch'^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'){throw 'LOCAL_OIDC_SYNTHETIC_SUPERVISOR_SUBJECT_INVALID'}
  $sql="UPDATE user_oidc_identities SET subject='$Subject',status='ACTIVE',revoked_at=NULL WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND user_id='90000000-0000-4000-8000-000000000013' AND oidc_provider_id='90000000-0000-4000-8000-000000000020';"
  $result=(& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc $sql|Out-String).Trim()
  if($LASTEXITCODE-ne 0-or$result-ne'UPDATE 1'){throw 'LOCAL_OIDC_SYNTHETIC_SUPERVISOR_SUBJECT_SYNC_FAILED'}
}
...
function Read-AssignmentOperationalSnapshot {
  $arguments=@('compose','--project-name',$projectName,'--env-file',$envFile)+$composeFiles+@('exec','-T','postgres','psql','--username','zap_pronto_owner','--dbname','zap_pronto','--tuples-only','--no-align','--command',
    "SELECT encode(digest(jsonb_build_object('availability',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.user_id,row.unit_id) FROM (SELECT user_id,unit_id,status,max_active,pause_reason,paused_until,version,updated_at FROM attendant_unit_availability WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'handoffs',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,assigned_user_id,version,claimed_at,queued_at,resolved_at FROM human_handoffs WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'conversations',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,automation_status,assigned_user_id,version,updated_at FROM conversations WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'cases',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,version,resolved_at FROM service_cases WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'messages',(SELECT jsonb_agg(jsonb_build_array(id,direction,actor,delivery_status) ORDER BY id) FROM messages WHERE tenant_id='90000000-0000-4000-8000-000000000001'),'outbox',(SELECT jsonb_agg(jsonb_build_array(id,status,event_type,attempts) ORDER BY id) FROM outbox_events WHERE tenant_id='90000000-0000-4000-8000-000000000001'))::text,'sha256'),'hex')")
  $value=(& docker @arguments|Out-String).Trim();if($LASTEXITCODE-ne 0-or$value-notmatch'^[a-f0-9]{64}$'){throw 'LOCAL_OIDC_ASSIGNMENT_SNAPSHOT_FAILED'};$value
}
function Setup {
  ...
  if(-not(Test-Path -LiteralPath $envFile)){
    $pg=New-RandomSecret;$runtime=New-RandomSecret;$admin=New-RandomSecret;$attendant=New-RandomSecret;$attendantTwo=New-RandomSecret;$supervisor=New-RandomSecret
  ...
  if(-not $existing.ContainsKey('LOCAL_OIDC_SUPERVISOR_PASSWORD')){
    Add-Content -LiteralPath $envFile -Value "LOCAL_OIDC_SUPERVISOR_PASSWORD=$(New-RandomSecret)"
    $existing=Read-LocalEnvironment
  }
...
function Up {
  ...
  Compose @('up','-d','--remove-orphans','--wait');Verify;$upValues=Read-LocalEnvironment
  $supervisorSubject=Reconcile-SyntheticSupervisor $upValues;Compose @('run','--rm','local-seed');Sync-SyntheticSupervisorSubject $supervisorSubject
}
function E2E {
  Assert-LocalPrerequisites -ForE2E
  $v=Read-LocalEnvironment;Assert-HarnessMarker;Assert-ProjectResources;$supervisorSubject=Reconcile-SyntheticSupervisor $v
  $hadE2EForbidSkips=Test-Path 'Env:E2E_FORBID_SKIPS';$previousE2EForbidSkips=[Environment]::GetEnvironmentVariable('E2E_FORBID_SKIPS','Process')
  $env:E2E_FORBID_SKIPS='true'
  $vars=@{E2E_OIDC_ENABLED='true';E2E_OIDC_TARGET='local';E2E_LOCAL_DESTRUCTIVE_ALLOWED='true';E2E_LOCAL_INSTANCE_NONCE=$v.LOCAL_HARNESS_NONCE;E2E_BASE_URL=$origin;E2E_REQUIRE_RENEWAL='true';E2E_REQUIRE_BLOCK_REVOCATION='true';E2E_RENEW_WAIT_SECONDS='45';E2E_OIDC_TEST_TIMEOUT_MS='240000';E2E_OIDC_USERNAME_SELECTOR='#username';E2E_OIDC_PASSWORD_SELECTOR='#password';E2E_OIDC_SUBMIT_SELECTOR='#kc-login';E2E_ADMIN_USERNAME='admin.local';E2E_ADMIN_PASSWORD=$v.LOCAL_OIDC_ADMIN_PASSWORD;E2E_ADMIN_EXPECTED_TENANT='Clínica Local';E2E_ATTENDANT_USERNAME='attendant.local';E2E_ATTENDANT_PASSWORD=$v.LOCAL_OIDC_ATTENDANT_PASSWORD;E2E_ATTENDANT_EXPECTED_TENANT='Clínica Local';E2E_ATTENDANT_TWO_USERNAME='attendant.two.local';E2E_ATTENDANT_TWO_PASSWORD=$v.LOCAL_OIDC_ATTENDANT_TWO_PASSWORD;E2E_ATTENDANT_TWO_EXPECTED_TENANT='Clínica Local';E2E_ATTENDANT_ADMIN_LIST_MATCH='attendant.local@example.test';E2E_ATTENDANT_TWO_ADMIN_LIST_MATCH='attendant.two.local@example.test';E2E_MANAGER_USERNAME='attendant.two.local';E2E_MANAGER_PASSWORD=$v.LOCAL_OIDC_ATTENDANT_TWO_PASSWORD;E2E_MANAGER_EXPECTED_TENANT='Clínica Local';E2E_MANAGER_MEMBERSHIP_MATCH='attendant.local';E2E_SUPERVISOR_USERNAME='supervisor.local';E2E_SUPERVISOR_PASSWORD=$v.LOCAL_OIDC_SUPERVISOR_PASSWORD;E2E_SUPERVISOR_EXPECTED_TENANT='Clínica Local'}
  try{foreach($entry in $vars.GetEnumerator()){Set-Item "Env:$($entry.Key)" $entry.Value};Compose @('run','--rm','local-seed');Sync-SyntheticSupervisorSubject $supervisorSubject
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor consulta disponibilidade da equipe sob demanda')
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor configura a primeira política de SLA uma única vez')
    $slaPolicyState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_sla_policy_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND version=1 AND created_by_user_id='90000000-0000-4000-8000-000000000012'; SELECT count(*) FROM unit_sla_policy_targets target JOIN unit_sla_policy_versions policy ON policy.tenant_id=target.tenant_id AND policy.id=target.policy_version_id WHERE target.tenant_id='90000000-0000-4000-8000-000000000001' AND policy.unit_id='90000000-0000-4000-8000-000000000002' AND (target.priority,target.target_minutes) IN(('LOW',240),('NORMAL',120),('HIGH',60),('URGENT',15)); SELECT count(*) FROM unit_sla_policy_publish_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND expected_version=0 AND result_version=1 AND actor_id='90000000-0000-4000-8000-000000000012'; SELECT count(*) FROM audit_events audit JOIN unit_sla_policy_versions policy ON policy.tenant_id=audit.tenant_id AND policy.id::text=audit.entity_id WHERE audit.tenant_id='90000000-0000-4000-8000-000000000001' AND policy.unit_id='90000000-0000-4000-8000-000000000002' AND policy.version=1 AND audit.action='SLA_POLICY_PUBLISHED' AND audit.entity_type='unit_sla_policy' AND audit.actor_id='90000000-0000-4000-8000-000000000012' AND audit.metadata->>'unitId'='90000000-0000-4000-8000-000000000002' AND audit.metadata->>'policyVersion'='1'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($slaPolicyState|Select-Object -First 1|Where-Object{$_-ne'1'}).Count-ne 0-or @($slaPolicyState|Select-Object -Skip 1 -First 1|Where-Object{$_-ne'4'}).Count-ne 0-or @($slaPolicyState|Select-Object -Skip 2 -First 2|Where-Object{$_-ne'1'}).Count-ne 0-or @($slaPolicyState|Select-Object -Skip 4|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_SLA_POLICY_STATE_INVALID'}
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor configura o fuso operacional uma única vez sem efeitos externos')
    $timezoneState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_operational_timezone_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND time_zone='America/Sao_Paulo' AND version=1 AND created_by_user_id='90000000-0000-4000-8000-000000000012'; SELECT count(*) FROM unit_operational_timezone_commands command JOIN unit_operational_timezone_versions version ON version.tenant_id=command.tenant_id AND version.id=command.result_version_id WHERE command.tenant_id='90000000-0000-4000-8000-000000000001' AND command.unit_id='90000000-0000-4000-8000-000000000002' AND command.time_zone='America/Sao_Paulo' AND command.expected_version=0 AND command.result_version=1 AND command.actor_id='90000000-0000-4000-8000-000000000012' AND version.version=1 AND version.time_zone='America/Sao_Paulo'; SELECT count(*) FROM audit_events audit JOIN unit_operational_timezone_versions version ON version.tenant_id=audit.tenant_id AND version.id::text=audit.entity_id WHERE audit.tenant_id='90000000-0000-4000-8000-000000000001' AND version.unit_id='90000000-0000-4000-8000-000000000002' AND version.version=1 AND version.time_zone='America/Sao_Paulo' AND audit.action='UNIT_OPERATIONAL_TIMEZONE_CONFIGURED' AND audit.entity_type='unit_operational_timezone' AND audit.actor_id='90000000-0000-4000-8000-000000000012' AND audit.metadata->>'unitId'='90000000-0000-4000-8000-000000000002' AND audit.metadata->>'timeZone'='America/Sao_Paulo' AND audit.metadata->>'version'='1'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($timezoneState|Select-Object -First 3|Where-Object{$_-ne'1'}).Count-ne 0-or @($timezoneState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_OPERATIONAL_TIMEZONE_STATE_INVALID'}
    $scheduleOperationalBefore=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT encode(digest(jsonb_build_object('availability',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.user_id,row.unit_id) FROM (SELECT user_id,unit_id,status,max_active,pause_reason,paused_until,version,updated_at FROM attendant_unit_availability WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'handoffs',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,assigned_user_id,version,claimed_at,queued_at,resolved_at FROM human_handoffs WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'conversations',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,automation_status,assigned_user_id,version,updated_at FROM conversations WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'cases',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,version,resolved_at FROM service_cases WHERE tenant_id='90000000-0000-4000-8000-000000000001') row))::text,'sha256'),'hex')"
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor publica uma escala semanal observacional uma única vez')
    Sync-SyntheticSupervisorSubject $supervisorSubject
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','supervisor consulta escala efetiva sem controles nem mutações')
    $scheduleOperationalAfter=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT encode(digest(jsonb_build_object('availability',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.user_id,row.unit_id) FROM (SELECT user_id,unit_id,status,max_active,pause_reason,paused_until,version,updated_at FROM attendant_unit_availability WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'handoffs',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,assigned_user_id,version,claimed_at,queued_at,resolved_at FROM human_handoffs WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'conversations',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,automation_status,assigned_user_id,version,updated_at FROM conversations WHERE tenant_id='90000000-0000-4000-8000-000000000001') row),'cases',(SELECT jsonb_agg(to_jsonb(row) ORDER BY row.id) FROM (SELECT id,status,version,resolved_at FROM service_cases WHERE tenant_id='90000000-0000-4000-8000-000000000001') row))::text,'sha256'),'hex')"
    if(($scheduleOperationalAfter|Out-String).Trim()-ne($scheduleOperationalBefore|Out-String).Trim()){throw 'LOCAL_OIDC_SHIFT_SCHEDULE_MUTATED_OPERATIONAL_STATE'}
    $scheduleState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_shift_schedule_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000011' AND version=1 AND effective_from BETWEEN current_date AND current_date+365 AND time_zone='America/Sao_Paulo' AND weekly_slots='[{`"weekday`": 1, `"start`": `"08:00`", `"end`": `"12:00`"}]'::jsonb AND jsonb_array_length(exceptions)=1 AND exceptions->0->>'type'='CLOSED' AND (exceptions->0->>'date')::date BETWEEN effective_from AND effective_from+365; SELECT count(*) FROM unit_shift_schedule_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000011' AND expected_version=0 AND result_version=1 AND actor_id='90000000-0000-4000-8000-000000000012'; SELECT count(*) FROM audit_events WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND action='SHIFT_SCHEDULE_PUBLISHED' AND entity_type='unit_shift_schedule' AND actor_id='90000000-0000-4000-8000-000000000012' AND metadata->>'unitId'='90000000-0000-4000-8000-000000000002' AND metadata->>'userId'='90000000-0000-4000-8000-000000000011' AND metadata->>'version'='1'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($scheduleState|Select-Object -First 3|Where-Object{$_-ne'1'}).Count-ne 0-or @($scheduleState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_SHIFT_SCHEDULE_STATE_INVALID'}
    Compose @('run','--rm','local-seed');Sync-SyntheticSupervisorSubject $supervisorSubject
    $assignmentFixture=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -v ON_ERROR_STOP=1 -U zap_pronto_owner -d zap_pronto -Atc "INSERT INTO unit_operational_timezone_versions(tenant_id,unit_id,time_zone,version,created_by_user_id) VALUES('90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002','UTC',1,'90000000-0000-4000-8000-000000000012'); INSERT INTO unit_shift_schedule_versions(tenant_id,unit_id,user_id,version,effective_from,time_zone,weekly_slots,exceptions,created_by_user_id) SELECT '90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002',member.user_id,1,current_date,'UTC',CASE WHEN member.user_id='90000000-0000-4000-8000-000000000011'::uuid THEN '[]'::jsonb ELSE '[{`"weekday`":1,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":2,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":3,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":4,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":5,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":6,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":7,`"start`":`"00:00`",`"end`":`"23:59`"}]'::jsonb END,'[]'::jsonb,'90000000-0000-4000-8000-000000000012' FROM user_units member WHERE member.tenant_id='90000000-0000-4000-8000-000000000001' AND member.unit_id='90000000-0000-4000-8000-000000000002' AND member.status='ACTIVE' AND member.role IN('TENANT_ADMIN','UNIT_MANAGER','SUPERVISOR','ATTENDANT');"
    if($LASTEXITCODE-ne 0){throw 'LOCAL_OIDC_ASSIGNMENT_POLICY_FIXTURE_FAILED'}
    $assignmentBefore=Read-AssignmentOperationalSnapshot
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor exige turno, atendente fora do turno é negado')
    $assignmentAfter=Read-AssignmentOperationalSnapshot;if($assignmentAfter-ne$assignmentBefore){throw 'LOCAL_OIDC_ASSIGNMENT_POLICY_MUTATED_INBOX'}
    $assignmentState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_assignment_policies WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND mode='OBSERVE' AND version=3; SELECT count(*) FROM unit_assignment_policy_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002'; SELECT count(*) FROM audit_events WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND action='UNIT_ASSIGNMENT_POLICY_CHANGED' AND metadata->>'unitId'='90000000-0000-4000-8000-000000000002'; SELECT count(*) FROM handoff_claim_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($assignmentState|Select-Object -First 1|Where-Object{$_-ne'1'}).Count-ne 0-or @($assignmentState|Select-Object -Skip 1 -First 2|Where-Object{$_-ne'2'}).Count-ne 0-or @($assignmentState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_ASSIGNMENT_POLICY_STATE_INVALID'}
    $timezoneMismatchFixture=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -v ON_ERROR_STOP=1 -U zap_pronto_owner -d zap_pronto -Atc "UPDATE unit_assignment_policies SET mode='ENFORCE_NEW_ASSIGNMENTS',version=4,updated_at=clock_timestamp() WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002';"
    if($LASTEXITCODE-ne 0){throw 'LOCAL_OIDC_TIMEZONE_MISMATCH_FIXTURE_FAILED'}
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','mudança de fuso invalida escalas antigas')
    $timezoneMismatchState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_operational_timezone_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND version=2 AND time_zone='America/Sao_Paulo'; SELECT count(*) FROM unit_shift_schedule_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND time_zone='America/Sao_Paulo'; SELECT count(*) FROM unit_assignment_policies WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND mode='ENFORCE_NEW_ASSIGNMENTS'; SELECT count(*) FROM handoff_claim_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($timezoneMismatchState|Select-Object -First 1|Where-Object{$_-ne'1'}).Count-ne 0-or @($timezoneMismatchState|Select-Object -Skip 1 -First 1|Where-Object{$_-ne'0'}).Count-ne 0-or @($timezoneMismatchState|Select-Object -Skip 2 -First 1|Where-Object{$_-ne'1'}).Count-ne 0-or @($timezoneMismatchState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_TIMEZONE_MISMATCH_STATE_INVALID'}
    Compose @('run','--rm','local-seed');Sync-SyntheticSupervisorSubject $supervisorSubject
    $transferEnforcementFixture=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -v ON_ERROR_STOP=1 -U zap_pronto_owner -d zap_pronto -Atc "INSERT INTO unit_operational_timezone_versions(tenant_id,unit_id,time_zone,version,created_by_user_id) VALUES('90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002','UTC',1,'90000000-0000-4000-8000-000000000012'); INSERT INTO unit_shift_schedule_versions(tenant_id,unit_id,user_id,version,effective_from,time_zone,weekly_slots,exceptions,created_by_user_id) SELECT '90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002',member.user_id,1,current_date,'UTC','[{`"weekday`":1,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":2,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":3,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":4,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":5,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":6,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":7,`"start`":`"00:00`",`"end`":`"23:59`"}]'::jsonb,'[]'::jsonb,'90000000-0000-4000-8000-000000000012' FROM user_units member WHERE member.tenant_id='90000000-0000-4000-8000-000000000001' AND member.unit_id='90000000-0000-4000-8000-000000000002' AND member.status='ACTIVE' AND member.role IN('TENANT_ADMIN','UNIT_MANAGER','SUPERVISOR','ATTENDANT');"
    if($LASTEXITCODE-ne 0){throw 'LOCAL_OIDC_TRANSFER_ENFORCEMENT_FIXTURE_FAILED'}
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','enforcement filtra destino fora do turno')
    $transferEnforcementState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM human_handoffs handoff JOIN conversations conversation ON conversation.tenant_id=handoff.tenant_id AND conversation.id=handoff.conversation_id JOIN service_cases service_case ON service_case.tenant_id=handoff.tenant_id AND service_case.id=handoff.service_case_id WHERE handoff.id='90000000-0000-4000-8000-000000000060' AND handoff.status='ACTIVE' AND handoff.assigned_user_id='90000000-0000-4000-8000-000000000011' AND handoff.version=2 AND conversation.status='OPEN' AND conversation.automation_status='HUMAN_ACTIVE' AND conversation.assigned_user_id='90000000-0000-4000-8000-000000000011' AND conversation.version=2 AND service_case.status='IN_REVIEW' AND service_case.version=2; SELECT count(*) FROM handoff_transfer_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM outbox_events WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND aggregate_id='90000000-0000-4000-8000-000000000060' AND event_type='handoff.transferred'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($transferEnforcementState|Select-Object -First 1|Where-Object{$_-ne'1'}).Count-ne 0-or @($transferEnforcementState|Select-Object -Skip 1|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_TRANSFER_ENFORCEMENT_STATE_INVALID'}
    Compose @('run','--rm','local-seed');Sync-SyntheticSupervisorSubject $supervisorSubject
    $slaShiftFixture=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -v ON_ERROR_STOP=1 -U zap_pronto_owner -d zap_pronto -Atc "INSERT INTO unit_operational_timezone_versions(tenant_id,unit_id,time_zone,version,created_by_user_id) VALUES('90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002','UTC',1,'90000000-0000-4000-8000-000000000012'); INSERT INTO unit_shift_schedule_versions(tenant_id,unit_id,user_id,version,effective_from,time_zone,weekly_slots,exceptions,created_by_user_id) SELECT '90000000-0000-4000-8000-000000000001','90000000-0000-4000-8000-000000000002',member.user_id,1,current_date,'UTC','[{`"weekday`":1,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":2,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":3,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":4,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":5,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":6,`"start`":`"00:00`",`"end`":`"23:59`"},{`"weekday`":7,`"start`":`"00:00`",`"end`":`"23:59`"}]'::jsonb,'[]'::jsonb,'90000000-0000-4000-8000-000000000012' FROM user_units member WHERE member.tenant_id='90000000-0000-4000-8000-000000000001' AND member.unit_id='90000000-0000-4000-8000-000000000002' AND member.status='ACTIVE' AND member.role IN('TENANT_ADMIN','UNIT_MANAGER','SUPERVISOR','ATTENDANT'); UPDATE attendant_unit_availability SET status='OFFLINE',version=version+1,updated_at=clock_timestamp() WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id<>'90000000-0000-4000-8000-000000000012';"
    if($LASTEXITCODE-ne 0){throw 'LOCAL_OIDC_SLA_SHIFT_CAPACITY_FIXTURE_FAILED'}
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','alerta SLA preserva o item e zera a capacidade')
    $slaShiftState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM unit_shift_schedule_versions WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000012' AND version=2 AND exceptions @> jsonb_build_array(jsonb_build_object('date',current_date::text,'type','CLOSED')); SELECT count(*) FROM unit_shift_schedule_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000012' AND expected_version=1 AND result_version=2; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($slaShiftState|Select-Object -First 2|Where-Object{$_-ne'1'}).Count-ne 0-or @($slaShiftState|Select-Object -Skip 2|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_SLA_SHIFT_CAPACITY_STATE_INVALID'}
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','gestor reconhece alerta de SLA uma única vez por versão')
    $slaAlertState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT (count(*)=2 AND count(*) FILTER(WHERE handoff_version=1)=1 AND count(*) FILTER(WHERE handoff_version=3)=1)::int FROM handoff_sla_acknowledgements WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060' AND acknowledged_by_user_id='90000000-0000-4000-8000-000000000012' AND handoff_version IN(1,3) AND version=1; SELECT (count(*)=2 AND count(*) FILTER(WHERE expected_version=1)=1 AND count(*) FILTER(WHERE expected_version=3)=1)::int FROM handoff_sla_acknowledge_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060' AND actor_id='90000000-0000-4000-8000-000000000012' AND expected_version IN(1,3); SELECT (count(*)=2 AND count(*) FILTER(WHERE metadata->>'handoffVersion'='1')=1 AND count(*) FILTER(WHERE metadata->>'handoffVersion'='3')=1)::int FROM audit_events WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND action='SLA_ALERT_ACKNOWLEDGED' AND entity_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM human_handoffs WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND id='90000000-0000-4000-8000-000000000060' AND status='QUEUED' AND version=3 AND assigned_user_id IS NULL AND claimed_at IS NULL; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($slaAlertState|Select-Object -First 4|Where-Object{$_-ne'1'}).Count-ne 0-or @($slaAlertState|Select-Object -Skip 4|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_SLA_ALERT_ACKNOWLEDGEMENT_STATE_INVALID'}
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','atendente altera a própria disponibilidade')
    $availabilityState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM attendant_unit_availability WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000011' AND status='PAUSED' AND max_active=7 AND pause_reason='BREAK' AND paused_until IS NULL AND version=2; SELECT count(*) FROM attendant_availability_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000011' AND requested_status='PAUSED' AND requested_max_active=7 AND requested_pause_reason='BREAK'"
    if(@($availabilityState|Where-Object{$_-ne'1'}).Count-ne 0){throw 'LOCAL_OIDC_ATTENDANT_AVAILABILITY_STATE_INVALID'}
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','administrador revoga e reativa vínculo, força OFFLINE')
    $membershipAvailabilityState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM user_units WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000012' AND status='ACTIVE'; SELECT count(*) FROM attendant_unit_availability WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND unit_id='90000000-0000-4000-8000-000000000002' AND user_id='90000000-0000-4000-8000-000000000012' AND status='OFFLINE' AND max_active=100 AND pause_reason IS NULL AND paused_until IS NULL; SELECT count(*) FROM handoff_claim_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM messages WHERE direction='OUTBOUND' OR actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(send|outbound|hermes|meta)'"
    if(@($membershipAvailabilityState|Select-Object -First 2|Where-Object{$_-ne'1'}).Count-ne 0-or @($membershipAvailabilityState|Select-Object -Skip 2|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_MEMBERSHIP_AVAILABILITY_STATE_INVALID'}
    Compose @('run','--rm','local-seed');$before=Read-InboxSnapshot;$beforeImmutable=Read-InboxImmutableSnapshot
    ...
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep','duas sessões convergem após hidden e offline')
    $automaticConvergenceState=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM messages WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND direction='OUTBOUND' AND actor='HUMAN' AND body='Convergência externa sintética' AND delivery_status='QUEUED'; SELECT count(*) FROM outbox_events event JOIN messages message ON message.tenant_id=event.tenant_id AND message.id=event.aggregate_id WHERE message.body='Convergência externa sintética' AND event.event_type='channel.outbound.requested'; SELECT count(*) FROM handoff_claim_commands WHERE tenant_id='90000000-0000-4000-8000-000000000001' AND handoff_id='90000000-0000-4000-8000-000000000060'; SELECT count(*) FROM messages WHERE actor='HERMES'; SELECT count(*) FROM outbox_events WHERE event_type~*'(hermes|meta)'"
    if(@($automaticConvergenceState|Select-Object -First 3|Where-Object{$_-ne'1'}).Count-ne 0-or @($automaticConvergenceState|Select-Object -Skip 3|Where-Object{$_-ne'0'}).Count-ne 0){throw 'LOCAL_OIDC_AUTOMATIC_CONVERGENCE_STATE_INVALID'}
    ...
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep-invert','(gestor consulta disponibilidade da equipe sob demanda|gestor configura a primeira política de SLA uma única vez|gestor configura o fuso operacional uma única vez sem efeitos externos|gestor publica uma escala semanal observacional uma única vez|supervisor consulta escala efetiva sem controles nem mutações|gestor exige turno, atendente fora do turno é negado|mudança de fuso invalida escalas antigas|enforcement filtra destino fora do turno|alerta SLA preserva o item e zera a capacidade|gestor reconhece alerta de SLA uma única vez|atendente altera a própria disponibilidade|administrador revoga e reativa vínculo, força OFFLINE|inbound materializado|duas sessões convergem após hidden e offline|resposta humana TEXT|cancelamento local mantém TEXT|atendente encerra o próprio atendimento|gestor consulta atendimento encerrado|gestor reabre atendimento encerrado uma única vez|transfere atendimento entre dois atendentes|gestor transfere e reconcilia supervisão|gestor assume atendimento supervisionado|reconciliação sintética local|gestor administra vínculos da unidade|admin encaminha entrada sem unidade)')
    $active=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM users WHERE email IN ('admin.local@example.test','attendant.local@example.test','attendant.two.local@example.test','supervisor.local@example.test') AND status='ACTIVE'";if(($active|Out-String).Trim()-ne'4'){throw 'LOCAL_OIDC_SYNTHETIC_ACCOUNTS_NOT_ACTIVE'}
    $activeMemberships=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM user_units membership JOIN users account ON account.tenant_id=membership.tenant_id AND account.id=membership.user_id WHERE account.email IN ('admin.local@example.test','attendant.local@example.test','attendant.two.local@example.test','supervisor.local@example.test') AND membership.status='ACTIVE'";if(($activeMemberships|Out-String).Trim()-ne'4'){throw 'LOCAL_OIDC_SYNTHETIC_MEMBERSHIPS_NOT_ACTIVE'}
    ...
    $supervisorMembership=& docker compose --project-name $projectName --env-file $envFile @composeFiles exec -T postgres psql -U zap_pronto_owner -d zap_pronto -Atc "SELECT count(*) FROM user_units membership JOIN users account ON account.tenant_id=membership.tenant_id AND account.id=membership.user_id WHERE account.id='90000000-0000-4000-8000-000000000013' AND account.email='supervisor.local@example.test' AND account.status='ACTIVE' AND membership.status='ACTIVE' AND membership.role='SUPERVISOR'";if(($supervisorMembership|Out-String).Trim()-ne'1'){throw 'LOCAL_OIDC_SYNTHETIC_SUPERVISOR_MEMBERSHIP_INVALID'}
  }finally{Compose @('run','--rm','local-seed');foreach($key in $vars.Keys){Remove-Item "Env:$key" -ErrorAction SilentlyContinue};if($hadE2EForbidSkips){Set-Item 'Env:E2E_FORBID_SKIPS' $previousE2EForbidSkips}else{Remove-Item 'Env:E2E_FORBID_SKIPS' -ErrorAction SilentlyContinue}}
}
function Untrust {

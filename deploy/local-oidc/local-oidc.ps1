...
function E2E {
  ...
  try{foreach($entry in $vars.GetEnumerator()){Set-Item "Env:$($entry.Key)" $entry.Value};Compose @('run','--rm','local-seed')
    ...
    Compose @('run','--rm','local-seed')
    Invoke-Checked 'pnpm' @('--filter','@zap-pronto/web','test:e2e:oidc','--grep-invert','(atendente altera a própria disponibilidade|inbound materializado|resposta humana TEXT|cancelamento local mantém TEXT|atendente encerra o próprio atendimento|gestor consulta atendimento encerrado|gestor reabre atendimento encerrado uma única vez|transfere atendimento entre dois atendentes|gestor transfere e reconcilia supervisão|gestor assume atendimento supervisionado|reconciliação sintética local|gestor administra vínculos da unidade|admin encaminha entrada sem unidade)')

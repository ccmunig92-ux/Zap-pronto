...
export function createApiClient(options: ApiClientOptions) {
  ...
  return { async getCurrentUser(): Promise<CurrentUser> {
    ...
    return data;
  }, async setChannelConnectionMetadata(input: ChannelConnectionMetadataRequest, idempotencyKey: string): Promise<ChannelConnectionMetadataResponse> {
    const token = await options.getAccessToken();
    if (!token) throw new AuthenticationRequired();
    const { data, error, response } = await client.POST("/v1/channel-connections", {
      headers: { authorization: `Bearer ${token}`, accept: "application/json" },
      params: { header: { "idempotency-key": idempotencyKey } }, body: input,
    });
    if (!response.ok) mapFailure(error, response);
    if (!Value.Check(ChannelConnectionMetadataResponseSchema, data)) throw new InvalidApiResponse();
    return data;

...
test("availability client uses unit query and stable command key",async()=>{const requests:Request[]=[];const unitId="33333333-3333-4333-8333-333333333333",userId="22222222-2222-4222-8222-222222222222",base={unitId,userId,status:"OFFLINE" as const,maxActive:5,pauseReason:null,pausedUntil:null,activeCount:0,version:1,updatedAt:"2026-08-12T20:00:00.000Z"};const client=createApiClient({baseUrl:"https://api.example.test",getAccessToken:async()=>"token",fetch:async request=>{requests.push(request);return Response.json(request.method==="GET"?base:{...base,status:"AVAILABLE",version:2,replayed:false})}});assert.equal((await client.getInboxAvailability(unitId)).status,"OFFLINE");await client.setInboxAvailability({unitId,status:"AVAILABLE",maxActive:5,expectedVersion:1},"availability-command-1");assert.equal(new URL(requests[0]!.url).searchParams.get("unitId"),unitId);assert.equal(requests[1]!.headers.get("idempotency-key"),"availability-command-1");assert.deepEqual(await requests[1]!.json(),{unitId,status:"AVAILABLE",maxActive:5,expectedVersion:1});});
...
test("SLA policy client uses unit path and stable command key",async()=>{const requests:Request[]=[];const unitId="33333333-3333-4333-8333-333333333333",targets=[{priority:"LOW"as const,targetMinutes:240},{priority:"NORMAL"as const,targetMinutes:120},{priority:"HIGH"as const,targetMinutes:60},{priority:"URGENT"as const,targetMinutes:15}],base={unitId,version:1,effectiveAt:"2026-08-13T12:00:00.000Z",targets};const client=createApiClient({baseUrl:"https://api.example.test",getAccessToken:async()=>"token",fetch:async request=>{requests.push(request);return Response.json(request.method==="GET"?base:{...base,version:2,replayed:false})}});assert.equal((await client.getUnitSlaPolicy(unitId)).version,1);await client.setUnitSlaPolicy(unitId,{expectedVersion:1,targets},"sla-policy-command-1");assert.equal(new URL(requests[0]!.url).pathname,`/v1/units/${unitId}/sla-policy`);assert.equal(requests[1]!.headers.get("idempotency-key"),"sla-policy-command-1");assert.deepEqual(await requests[1]!.json(),{expectedVersion:1,targets});});

test("capacity alert client keeps policy commands scoped and snapshot read-only",async()=>{const requests:Request[]=[];const unitId="33333333-3333-4333-8333-333333333333",policy={unitId,mode:"ENABLED"as const,minimumQueued:3,sustainedMinutes:10,version:1,updatedAt:"2026-08-19T12:00:00.000Z"},snapshot={unitId,policyVersion:1,enabled:true,minimumQueued:3,sustainedMinutes:10,queuedCount:4,sustainedQueuedCount:3,oldestQueuedAt:"2026-08-19T11:45:00.000Z",availableCapacity:2,state:"ACTIVE"as const,evaluatedAt:"2026-08-19T12:00:00.000Z"};const client=createApiClient({baseUrl:"https://api.example.test",getAccessToken:async()=>"token",fetch:async request=>{requests.push(request);const url=new URL(request.url);if(url.pathname==="/v1/inbox/capacity-alert")return Response.json(snapshot);return Response.json(request.method==="GET"?policy:{...policy,version:2,replayed:false})}});assert.equal((await client.getUnitCapacityAlertPolicy(unitId)).mode,"ENABLED");await client.setUnitCapacityAlertPolicy(unitId,{expectedVersion:1,mode:"ENABLED",minimumQueued:3,sustainedMinutes:10},"capacity-alert-command-1");assert.equal((await client.getInboxCapacityAlert(unitId)).state,"ACTIVE");assert.equal(new URL(requests[0]!.url).pathname,`/v1/units/${unitId}/capacity-alert-policy`);assert.equal(requests[1]!.headers.get("idempotency-key"),"capacity-alert-command-1");assert.deepEqual(await requests[1]!.json(),{expectedVersion:1,mode:"ENABLED",minimumQueued:3,sustainedMinutes:10});const snapshotRequest=requests[2]!;assert.equal(snapshotRequest.method,"GET");assert.equal(new URL(snapshotRequest.url).searchParams.get("unitId"),unitId);});

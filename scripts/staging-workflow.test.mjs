...
test("external Inbox fixture is isolated behind pinned SSH and always cleaned", () => {
  ...
  assert.match(oidcSpec, /withPreservedPrimaryFailure<T>[\s\S]*primaryFailure=error;throw error[\s\S]*if\(primaryFailure===undefined\)throw recoveryError/);
  ...
  assert.match(oidcSpec, /safeActiveListDiagnostic[\s\S]*fixtureMatchCount/);
  assert.match(oidcSpec, /reloadAndLocateActiveFixture[\s\S]*\/v1\/inbox\/active[\s\S]*Atualizar Inbox/);
  assert.match(externalInboxSource, /const activeItem=await reloadAndLocateActiveFixture\(page,contactName\)/);
  assert.match(externalInboxSource, /recoverOwnExternalFixture\(page,contactName\)/);
  assert.match(externalInboxSource, /withPreservedPrimaryFailure\(async\(\)=>[\s\S]*recoverOwnExternalFixture\(page,contactName\)[\s\S]*restoreOwnAvailabilityOffline\(page\)/);

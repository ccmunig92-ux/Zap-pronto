...
if(document.paths?.["/v1/units/{unitId}/staff-schedules/members"]?.get?.operationId!=="listShiftMembers"||document.paths?.["/v1/units/{unitId}/staff-schedules/{userId}"]?.get?.operationId!=="getStaffSchedule"||document.paths?.["/v1/units/{unitId}/staff-schedules/{userId}"]?.post?.operationId!=="setStaffSchedule")throw new Error("OPENAPI_STAFF_SCHEDULE_MISSING");

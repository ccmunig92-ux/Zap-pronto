...
if(document.paths?.["/v1/inbox/capacity-alert-episodes"]?.get?.operationId!=="listCapacityAlertEpisodes"||document.paths?.["/v1/inbox/capacity-alert-episodes/{episodeId}/acknowledge"]?.post?.operationId!=="acknowledgeCapacityAlertEpisode")throw new Error("OPENAPI_CAPACITY_ALERT_EPISODES_MISSING");
...
if(document.paths?.["/v1/units/{unitId}/staff-schedules/members"]?.get?.operationId!=="listShiftMembers"||document.paths?.["/v1/units/{unitId}/staff-schedules/{userId}"]?.get?.operationId!=="getStaffSchedule"||document.paths?.["/v1/units/{unitId}/staff-schedules/{userId}"]?.post?.operationId!=="setStaffSchedule"||document.paths?.["/v1/units/{unitId}/staff-schedules/{userId}/effective"]?.get?.operationId!=="getEffectiveStaffShift")throw new Error("OPENAPI_STAFF_SCHEDULE_MISSING");

export class InboxHandoffRequestError extends Error {
  ...
  static from(error: unknown): never {
    ...
    if (["HANDOFF_TRANSFER_CONFLICT","HANDOFF_TRANSFER_IDEMPOTENCY_CONFLICT","ASSIGNEE_NOT_ELIGIBLE"].includes(code)) throw new InboxHandoffRequestError(409,"HANDOFF_CONFLICT");
    if (["HANDOFF_TAKEOVER_CONFLICT","HANDOFF_TAKEOVER_IDEMPOTENCY_CONFLICT","HANDOFF_TAKEOVER_PENDING_OUTBOUND"].includes(code)) throw new InboxHandoffRequestError(409,"HANDOFF_CONFLICT");
    throw error;
  }

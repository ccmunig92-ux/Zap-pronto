...
export interface InboundWorkerRuntimeConfig { databaseUrl:string;batchSize:number;leaseSeconds:number;
  ...
  capacityAlertPollIntervalMs:number;capacityAlertDiscoveryPageSize:number;
  capacityAlertDiscoveryFailureThreshold:number; }
...
export async function loadInboundWorkerRuntimeConfig(env:NodeJS.ProcessEnv=process.env):Promise<InboundWorkerRuntimeConfig>{
  ...
  return {databaseUrl:await secret(env),batchSize:integer(env,"INBOUND_WORKER_BATCH_SIZE",10,1,100),leaseSeconds,
    ...
    capacityAlertPollIntervalMs:integer(env,"CAPACITY_ALERT_POLL_INTERVAL_MS",60_000,1000,3_600_000),
    capacityAlertDiscoveryPageSize:integer(env,"CAPACITY_ALERT_DISCOVERY_PAGE_SIZE",100,1,100),
    capacityAlertDiscoveryFailureThreshold:integer(env,"CAPACITY_ALERT_DISCOVERY_FAILURE_THRESHOLD",3,1,100)};
}

...
test("health contract and OpenAPI are generated by the same schemas", async () => {
  const app = await buildApp({ releaseId: "release-test-123" });
  const response = await app.inject({ method: "GET", url: "/health/live" });
  assert.equal(response.statusCode, 200);
  assert.deepEqual(response.json(), { status: "ok" });
  assert.match(String(response.headers["x-correlation-id"]), /^[0-9a-f-]{36}$/);
  assert.equal(response.headers["x-zap-pronto-release"], "release-test-123");
...
test("runtime access logs use route templates and never serialize credentials or query values", async () => {
  const lines: string[] = [];
  const app = await buildApp({ releaseId: "release-log-123", logger: {
    level: "info", stream: { write(line) { lines.push(line); } },
  } });
  await app.inject({ method: "GET", url: "/health/live?token=secret-query-value", headers: {
    authorization: "Bearer secret-access-token", cookie: "session=secret-cookie",
  } });
  await app.close();
  const output = lines.join("");
  assert.match(output, /http\.request/);
  assert.match(output, /http\.response/);
  assert.match(output, /\/health\/live/);
  assert.doesNotMatch(output, /secret-query-value|secret-access-token|secret-cookie|authorization|cookie/i);

  const failures: string[] = [];
  const failed = await buildApp({ logger: { level: "info", stream: { write(line) { failures.push(line); } } } });
  failed.get("/unexpected-log-test", async () => {
    const error = new Error("SECRET_INTERNAL_DETAIL"); error.name = "SECRET_ERROR_NAME"; throw error;
  });
  const response = await failed.inject({ method: "GET", url: "/unexpected-log-test?private=value" });
  await failed.close();
  assert.equal(response.statusCode, 500);
  const failureOutput = failures.join("");
  assert.match(failureOutput, /http\.error/);
  assert.match(failureOutput, /Unexpected|Error/);
  assert.doesNotMatch(failureOutput, /SECRET_INTERNAL_DETAIL|SECRET_ERROR_NAME|private=value/);
});
...
test("readiness checks the database and fails closed without exposing its error", async () => {
  ...
  const ready = await buildApp({ pool: { async connect() { return {
    async query(sql) { queries.push(sql); return { rows: sql.includes("current_user")
      ? [{ session_user: "zap_pronto_runtime", current_user: "zap_pronto_api" }] : [] }; },
    release() { releases += 1; },
  }; } } });
  const healthy = await ready.inject({ method: "GET", url: "/health/ready" });
  assert.equal(healthy.statusCode, 200);
  assert.deepEqual(healthy.json(), { status: "ok" });
  assert.deepEqual(queries, ["BEGIN READ ONLY", "SET LOCAL ROLE zap_pronto_api",
    "SELECT session_user AS session_user, current_user AS current_user", "ROLLBACK"]);
  ...
  const wrongRole = await buildApp({ pool: { async connect() { return {
    async query(sql) { return { rows: sql.includes("current_user")
      ? [{ session_user: "postgres", current_user: "zap_pronto_api" }] : [] }; },
    release() {},
  }; } } });
  const denied = await wrongRole.inject({ method: "GET", url: "/health/ready" });
  assert.equal(denied.statusCode, 503);
  assert.doesNotMatch(denied.body, /postgres|zap_pronto|DATABASE_RUNTIME_ROLE/);
...
test("every versioned route has an explicit policy, scope and coherent error contract", async () => {
  ...
  assert.deepEqual(policies.sort((left, right) => String(left).localeCompare(String(right))), [
    ...
    ["GET", "/v1/inbox/routing-required", "permission", "inbound.routing.read", "tenant"],
      ...
      ["GET", "/v1/units/:unitId/capacity-alert-policy", "permission", "sla_alert.read", "unit"],
      ["GET", "/v1/units/:unitId/assignment-policy", "permission", "shift.read", "unit"],
    ...
    ["GET", "/v1/units/:unitId/memberships", "permission", "unit.members.manage", "unit"],
      ...
      ["POST", "/v1/units/:unitId/capacity-alert-policy", "permission", "sla_alert.manage", "unit"],
      ["POST", "/v1/units/:unitId/assignment-policy", "permission", "shift.manage", "unit"],
      ["POST", "/v1/units/:unitId/operational-timezone", "permission", "unit_timezone.manage", "unit"],
      ["POST", "/v1/units/:unitId/staff-schedules/:userId", "permission", "shift.manage", "unit"],

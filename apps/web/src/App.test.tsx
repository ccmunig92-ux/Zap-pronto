...
describe("authenticated shell", () => {
  it("abre Canais diretamente pelo deep link canônico", async () => {
    window.history.replaceState({}, "", "/configuracoes/canais");
    const listChannelConnections = vi.fn(async () => ({ items: [] }));
    render(<App client={{ async getCurrentUser() { return {
      user: { id: "22222222-2222-4222-8222-222222222222", email: "admin@test", displayName: "Admin" },
      tenant: { id: "11111111-1111-4111-8111-111111111111", name: "Clínica" }, memberships: [],
      grants: [{ permission: "channel_connections.read" as const, scope: "TENANT" as const }],
    }; } }} connectionsClient={{ listChannelConnections }}/>)
    expect(await screen.findByRole("heading", { name: "Canais e integrações" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "Canais" }).getAttribute("aria-current")).toBe("page");
    await waitFor(() => expect(listChannelConnections).toHaveBeenCalledOnce());
  });
  it("monta Equipe uma única vez apenas com availability.supervise e unidades autorizadas",async()=>{const allowed="33333333-3333-4333-8333-333333333333",denied="44444444-4444-4444-8444-444444444444",list=vi.fn(async()=>({items:[]}));render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"supervisor@test",displayName:"Supervisor"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId:allowed,unitCode:"CENTRO",unitName:"Centro",role:"SUPERVISOR"as const},{unitId:denied,unitCode:"NORTE",unitName:"Norte",role:"SUPERVISOR"as const}],grants:[{permission:"availability.supervise"as const,scope:"UNIT"as const,unitId:allowed}]}}}} teamAvailabilityClient={{listInboxTeamAvailability:list}}/>);expect(list).not.toHaveBeenCalled();fireEvent.click(await screen.findByRole("button",{name:"Equipe"}));expect(await screen.findByRole("heading",{name:"Disponibilidade da equipe"})).toBeTruthy();expect(screen.getByRole("option",{name:"Centro"})).toBeTruthy();expect(screen.queryByRole("option",{name:"Norte"})).toBeNull();await waitFor(()=>expect(list).toHaveBeenCalledTimes(1));fireEvent.click(screen.getByRole("button",{name:"Equipe"}));expect(list).toHaveBeenCalledTimes(1)});
  it("monta Escalas sob demanda e separa leitura de edição",async()=>{const allowed="33333333-3333-4333-8333-333333333333",denied="44444444-4444-4444-8444-444444444444",get=vi.fn(async()=>({unitId:allowed,timeZone:"America/Sao_Paulo",version:1,updatedAt:"2026-08-13T12:00:00.000Z"})),set=vi.fn();render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"supervisor@test",displayName:"Supervisor"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId:allowed,unitCode:"CENTRO",unitName:"Centro",role:"SUPERVISOR"as const},{unitId:denied,unitCode:"NORTE",unitName:"Norte",role:"SUPERVISOR"as const}],grants:[{permission:"unit_timezone.read"as const,scope:"UNIT"as const,unitId:allowed}]}}}} operationalTimezoneClient={{getUnitOperationalTimezone:get,setUnitOperationalTimezone:set}}/>);expect(get).not.toHaveBeenCalled();fireEvent.click(await screen.findByRole("button",{name:"Escalas"}));expect(await screen.findByRole("heading",{name:"Fuso operacional"})).toBeTruthy();expect(screen.getByRole("option",{name:"Centro"})).toBeTruthy();expect(screen.queryByRole("option",{name:"Norte"})).toBeNull();expect(screen.queryByLabelText("Fuso IANA")).toBeNull();expect(get).toHaveBeenCalledTimes(1);expect(set).not.toHaveBeenCalled()});
  it("não monta política de atribuição com shift.manage sem shift.read",async()=>{const unitId="33333333-3333-4333-8333-333333333333",get=vi.fn();render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"manager@test",displayName:"Gerente"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId,unitCode:"CENTRO",unitName:"Centro",role:"UNIT_MANAGER"as const}],grants:[{permission:"shift.manage"as const,scope:"UNIT"as const,unitId}]}}}} assignmentPolicyClient={{getUnitAssignmentPolicy:get,setUnitAssignmentPolicy:vi.fn()}}/>);expect(await screen.findByText("Clínica")).toBeTruthy();expect(screen.queryByRole("button",{name:"Escalas"})).toBeNull();expect(screen.queryByRole("heading",{name:"Política de novas atribuições"})).toBeNull();expect(get).not.toHaveBeenCalled()});
  it("monta política SLA somente para unidade com grant explícito de gestão",async()=>{const allowed="33333333-3333-4333-8333-333333333333",denied="44444444-4444-4444-8444-444444444444",get=vi.fn(async()=>null);render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"manager@test",displayName:"Gerente"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId:allowed,unitCode:"CENTRO",unitName:"Centro",role:"UNIT_MANAGER"as const},{unitId:denied,unitCode:"NORTE",unitName:"Norte",role:"UNIT_MANAGER"as const}],grants:[{permission:"sla_policy.manage"as const,scope:"UNIT"as const,unitId:allowed}]}}}} slaPolicyClient={{getUnitSlaPolicy:get,async setUnitSlaPolicy(){throw new Error("not called")}}}/>);expect(await screen.findByRole("heading",{name:"Política de SLA da unidade"})).toBeTruthy();expect(screen.getByRole("option",{name:"Centro"})).toBeTruthy();expect(screen.queryByRole("option",{name:"Norte"})).toBeNull();await waitFor(()=>expect(get).toHaveBeenCalledWith(allowed))});
 it("permite ao administrador tenant configurar SLA nas próprias unidades ativas",async()=>{const unitId="33333333-3333-4333-8333-333333333333",get=vi.fn(async()=>null);render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"admin@test",displayName:"Admin"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId,unitCode:"CENTRO",unitName:"Centro",role:"TENANT_ADMIN"as const}],grants:[{permission:"sla_policy.manage"as const,scope:"TENANT"as const,unitId:null}]}}}} slaPolicyClient={{getUnitSlaPolicy:get,async setUnitSlaPolicy(){throw new Error("not called")}}}/>);expect(await screen.findByRole("heading",{name:"Política de SLA da unidade"})).toBeTruthy();await waitFor(()=>expect(get).toHaveBeenCalledWith(unitId))});
 it("monta política SLA somente para leitura do supervisor sem ação de salvar",async()=>{const unitId="33333333-3333-4333-8333-333333333333",get=vi.fn(async()=>({unitId,version:1,effectiveAt:"2026-08-13T10:00:00.000Z",targets:[{priority:"LOW"as const,targetMinutes:240},{priority:"NORMAL"as const,targetMinutes:120},{priority:"HIGH"as const,targetMinutes:60},{priority:"URGENT"as const,targetMinutes:15}]})),set=vi.fn();render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"supervisor@test",displayName:"Supervisor"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId,unitCode:"CENTRO",unitName:"Centro",role:"SUPERVISOR"as const}],grants:[{permission:"sla_policy.read"as const,scope:"UNIT"as const,unitId}]}}}} slaPolicyClient={{getUnitSlaPolicy:get,setUnitSlaPolicy:set}}/>);expect(await screen.findByRole("heading",{name:"Política de SLA da unidade"})).toBeTruthy();expect(await screen.findByText("240 minutos")).toBeTruthy();expect(screen.queryByRole("button",{name:"Salvar política de SLA"})).toBeNull();expect(set).not.toHaveBeenCalled()});
  it("não monta nem consulta política SLA sem capability",async()=>{const get=vi.fn(async()=>null);render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"agent@test",displayName:"Agente"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId:"33333333-3333-4333-8333-333333333333",unitCode:"CENTRO",unitName:"Centro",role:"ATTENDANT"as const}],grants:[]}}}} slaPolicyClient={{getUnitSlaPolicy:get,async setUnitSlaPolicy(){throw new Error("not called")}}}/>);expect(await screen.findByText("Clínica")).toBeTruthy();expect(screen.queryByRole("button",{name:"Política de SLA"})).toBeNull();expect(get).not.toHaveBeenCalled()});
...
it("exibe configuração agregada com grants sla_alert sem consultar política SLA",async()=>{const unitId="33333333-3333-4333-8333-333333333333",getSla=vi.fn(),getCapacity=vi.fn(async()=>({unitId,mode:"DISABLED"as const,minimumQueued:null,sustainedMinutes:null,version:0,updatedAt:null}));render(<App client={{async getCurrentUser(){return{user:{id:"22222222-2222-4222-8222-222222222222",email:"manager@test",displayName:"Gerente"},tenant:{id:"11111111-1111-4111-8111-111111111111",name:"Clínica"},memberships:[{unitId,unitCode:"CENTRO",unitName:"Centro",role:"UNIT_MANAGER"as const}],grants:[{permission:"sla_alert.read"as const,scope:"UNIT"as const,unitId},{permission:"sla_alert.manage"as const,scope:"UNIT"as const,unitId}]}}}} slaPolicyClient={{getUnitSlaPolicy:getSla,setUnitSlaPolicy:vi.fn(),getUnitCapacityAlertPolicy:getCapacity,setUnitCapacityAlertPolicy:vi.fn()}}/>);expect(await screen.findByRole("heading",{name:"Alerta agregado de demanda e capacidade"})).toBeTruthy();expect((await screen.findByRole("button",{name:"Salvar alerta agregado"})).hasAttribute("disabled")).toBe(true);expect(getCapacity).toHaveBeenCalledWith(unitId);expect(getSla).not.toHaveBeenCalled()});
});

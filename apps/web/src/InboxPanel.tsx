...
export function InboxPanel({ client, units, supervisedUnitIds=[], historyUnitIds=[], slaAlertReadUnitIds=[],slaAlertAcknowledgeUnitIds=[],onAuthenticationRequired,
  ...
  return <section className="inbox" aria-busy={refreshing}>
    ...
    {canReadSlaAlerts&&client.listCapacityAlertEpisodes&&<section className="inbox-queue-section" aria-labelledby="capacity-episodes-title"><div className="inbox-section-heading"><h3 id="capacity-episodes-title">Episódios de capacidade</h3><span aria-label={`${capacityEpisodes?.items.length??0} episódios de capacidade`}>{capacityEpisodes?.items.length??0}</span></div>
      ...
      <ul className="handoff-list">{capacityEpisodes?.items.map(episode=><li key={episode.episodeId}><strong>{capacityEpisodeStatusLabel(episode.status)}</strong><span>Nível de escalonamento: {episode.escalationLevel}</span><span>{episode.recipientCount} destinatários registrados</span><time dateTime={episode.openedAt}>Aberto em {new Date(episode.openedAt).toLocaleString("pt-BR")}</time>{episode.acknowledgedAt&&<span>Reconhecido em {new Date(episode.acknowledgedAt).toLocaleString("pt-BR")}</span>}{episode.acknowledgementReason&&<span>Motivo: {episode.acknowledgementReason}</span>}{canAcknowledgeSlaAlerts&&episode.status!=="ACKNOWLEDGED"&&episode.status!=="RESOLVED"&&<button type="button" disabled={mutationBusy||refreshing} onClick={()=>void acknowledgeCapacityEpisode(episode)}>{acknowledgingEpisodeId===episode.episodeId?"Reconhecendo…":"Reconhecer episódio"}</button>}</li>)}</ul>
    </section>}

...
describe("ConnectionsPanel", () => {
  ...
  it("reporta alterações de formulário e bloqueia navegação durante o salvamento", async () => {
    let resolveSave!: (value: { connection: typeof connection; replayed: boolean }) => void;
    const set = vi.fn().mockImplementation(() => new Promise(resolve => { resolveSave = resolve; }));
    const navigation = vi.fn();
    render(<ConnectionsPanel client={{ listChannelConnections: vi.fn().mockResolvedValue({ items: [] }), setChannelConnectionMetadata: set }} canManage onNavigationStateChange={navigation} />);
    fireEvent.click(await screen.findByRole("button", { name: "Configurar conexão" }));
    await waitFor(() => expect(navigation).toHaveBeenLastCalledWith({ blocked: false, dirty: false }));
    fireEvent.change(screen.getByLabelText("WABA ID"), { target: { value: "123456" } });
    expect(navigation).toHaveBeenLastCalledWith({ blocked: false, dirty: true });
    fireEvent.change(screen.getByLabelText("Phone Number ID"), { target: { value: "654321" } });
    fireEvent.change(screen.getByLabelText(/Referência protegida do segredo/), { target: { value: "meta.production" } });
    fireEvent.click(screen.getByRole("button", { name: "Salvar conexão" }));
    await waitFor(() => expect(navigation).toHaveBeenLastCalledWith({ blocked: true, dirty: true }));
    resolveSave({ connection, replayed: false });
    await waitFor(() => expect(navigation).toHaveBeenLastCalledWith({ blocked: false, dirty: false }));
  });

...
test.describe("shell OIDC real", () => {
  ...
  test("gestor exige turno, atendente fora do turno é negado e gestor retorna à observação",async({browser})=>{
    ...
    try{await login(managerPage,account("MANAGER"));await openModule(managerPage,"Escalas");await expect(managerPage.getByRole("heading",{name:"Política de novas atribuições"})).toBeVisible();await expect(managerPage.getByText(/4 de 4 integrantes operacionais/)).toBeVisible();
  ...
  test("mudança de fuso invalida escalas antigas e mantém enforcement fail-closed",async({browser})=>{
    ...
    try{await login(manager,account("MANAGER"));await openModule(manager,"Escalas");await expect(manager.getByText(/4 de 4 integrantes operacionais/)).toBeVisible();await manager.getByLabel("Fuso IANA").fill("America/Sao_Paulo");await manager.getByRole("button",{name:"Salvar fuso operacional"}).click();const changed=manager.waitForResponse(response=>response.request().method()==="POST"&&/\/operational-timezone$/u.test(new URL(response.url()).pathname));await manager.getByRole("button",{name:"Confirmar alteração"}).click();expect((await changed).status()).toBe(200);await expect(manager.getByText(/Fuso vigente: America\/Sao_Paulo/)).toBeVisible();await manager.getByRole("button",{name:"Atualizar política"}).click();await expect(manager.getByText(/0 de 4 integrantes operacionais possuem escala vigente; 4 pendentes/)).toBeVisible();

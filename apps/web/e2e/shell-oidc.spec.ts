...
test.describe("shell OIDC real", () => {
  ...
  test("gestor configura a primeira política de SLA uma única vez e persiste a versão",async({page})=>{
    test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar a política de SLA local.");await login(page,account("MANAGER"));
    const posts:string[]=[];const mutations:string[]=[];const externalHosts:string[]=[];page.on("request",request=>{const url=new URL(request.url());
      if(url.hostname!=="zap-pronto.127.0.0.1.nip.io")externalHosts.push(url.host);
      if(url.pathname.startsWith("/v1/")&&["POST","PATCH","PUT","DELETE"].includes(request.method()))mutations.push(`${request.method()} ${url.pathname}`);
      if(request.method()==="POST"&&/^\/v1\/units\/[^/]+\/sla-policy$/u.test(url.pathname))posts.push(url.pathname)});
    await openModule(page,"Política de SLA");await expect(page.getByRole("heading",{name:"Política de SLA da unidade"})).toBeVisible();
    await expect(page.getByText("Nenhuma política de SLA configurada para esta unidade.")).toBeVisible();
    await page.getByLabel("Prioridade baixa (minutos)").fill("240");await page.getByLabel("Prioridade normal (minutos)").fill("120");
    await page.getByLabel("Prioridade alta (minutos)").fill("60");await page.getByLabel("Prioridade urgente (minutos)").fill("15");
    await page.getByRole("button",{name:"Salvar política de SLA"}).click();await expect(page.getByRole("dialog",{name:"Confirmar política de SLA"})).toBeVisible();
    const saved=page.waitForResponse(response=>response.request().method()==="POST"&&/^\/v1\/units\/[^/]+\/sla-policy$/u.test(new URL(response.url()).pathname));
    ...
    expect(posts).toHaveLength(1);expect(mutations).toEqual([`POST ${posts[0]}`]);await expect(page.getByText("Política de SLA atualizada.")).toBeVisible();
    await expect(page.getByText(/Versão 1\./u)).toBeVisible();await page.reload();await openModule(page,"Política de SLA");
  ...
  test("gestor publica uma escala semanal observacional uma única vez",async({page})=>{
    ...
    const scheduleForm=page.getByRole("heading",{name:"Grade semanal e exceções"}).locator("xpath=ancestor::section[1]");await scheduleForm.getByLabel("Vigência a partir de").fill(effectiveFrom);await scheduleForm.getByRole("button",{name:"Adicionar período"}).click();await scheduleForm.getByLabel("Início").fill("08:00");await scheduleForm.getByLabel("Fim").fill("12:00");await scheduleForm.getByRole("button",{name:"Adicionar exceção"}).click();await scheduleForm.getByLabel("Data").fill(exceptionDate);await scheduleForm.getByRole("button",{name:"Salvar escala"}).click();const dialog=page.getByRole("dialog",{name:"Confirmar escala"});await expect(dialog).toBeVisible();const saved=page.waitForResponse(response=>response.request().method()==="POST"&&/^\/v1\/units\/[^/]+\/staff-schedules\/[^/]+$/u.test(new URL(response.url()).pathname));await dialog.getByRole("button",{name:"Confirmar alteração"}).evaluate((element:HTMLButtonElement)=>{element.click();element.click()});expect((await saved).status()).toBe(200);await expect(scheduleForm.getByText("Escala semanal atualizada.")).toBeVisible();await expect(scheduleForm.getByText(/Fuso da versão:/)).toBeVisible();await expect(scheduleForm.getByText("America/Sao_Paulo",{exact:true})).toBeVisible();
    await page.reload();await openModule(page,"Escalas");const schedulePanel=page.getByRole("heading",{name:"Grade semanal e exceções"}).locator("xpath=ancestor::section[1]");await schedulePanel.getByLabel("Integrante").selectOption({label:"Atendente Local · ATTENDANT"});await expect(schedulePanel.getByText(/Versão 1/)).toBeVisible();await expect(schedulePanel.getByLabel("Vigência a partir de")).toHaveValue(effectiveFrom);expect(posts).toHaveLength(1);expect(externalHosts).toEqual([]);
  });

  test("gestor exige turno, atendente fora do turno é negado e gestor retorna à observação",async({browser})=>{
    test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar enforcement local.");const managerPage=await browser.newPage(),attendantPage=await browser.newPage();const mutations:string[]=[],externalHosts:string[]=[];
    for(const page of[managerPage,attendantPage])page.on("request",request=>{const url=new URL(request.url());if(url.hostname!=="zap-pronto.127.0.0.1.nip.io")externalHosts.push(url.host);if(url.pathname.startsWith("/v1/")&&["POST","PATCH","PUT","DELETE"].includes(request.method()))mutations.push(`${request.method()} ${url.pathname}`)});
    try{await login(managerPage,account("MANAGER"));await openModule(managerPage,"Escalas");await expect(managerPage.getByRole("heading",{name:"Política de novas atribuições"})).toBeVisible();await expect(managerPage.getByText(/^\d+ de \d+ integrantes operacionais possuem escala vigente; \d+ pendentes\.$/u)).toBeVisible();
      await managerPage.getByLabel("Aplicação").selectOption("ENFORCE_NEW_ASSIGNMENTS");await managerPage.getByRole("button",{name:"Salvar política de atribuição"}).click();await expect(managerPage.getByText(/Novos claims e destinos de transferência exigirão turno vigente/)).toBeVisible();const enabledResponse=managerPage.waitForResponse(response=>response.request().method()==="POST"&&/\/assignment-policy$/u.test(new URL(response.url()).pathname));await managerPage.getByRole("button",{name:"Confirmar alteração"}).evaluate((element:HTMLButtonElement)=>{element.click();element.click()});expect((await enabledResponse).status()).toBe(200);await expect(managerPage.getByText("Exigência de turno ativada.")).toBeVisible();
      await login(attendantPage,account("ATTENDANT"));await attendantPage.getByRole("button",{name:"Contato · NORMAL"}).click();const denied=attendantPage.waitForResponse(response=>response.request().method()==="POST"&&/\/claim$/u.test(new URL(response.url()).pathname));await attendantPage.getByRole("button",{name:"Assumir atendimento"}).click();expect((await denied).status()).toBe(409);await expect(attendantPage.getByRole("alert")).toContainText("fora do turno configurado");await expect(attendantPage.getByRole("button",{name:"Contato · NORMAL"})).toBeVisible();
      await managerPage.getByLabel("Aplicação").selectOption("OBSERVE");await managerPage.getByRole("button",{name:"Salvar política de atribuição"}).click();const disabledResponse=managerPage.waitForResponse(response=>response.request().method()==="POST"&&/\/assignment-policy$/u.test(new URL(response.url()).pathname));await managerPage.getByRole("button",{name:"Confirmar alteração"}).evaluate((element:HTMLButtonElement)=>{element.click();element.click()});expect((await disabledResponse).status()).toBe(200);await expect(managerPage.getByText("Política mantida em observação.")).toBeVisible();
  ...
  test("resposta humana TEXT fica QUEUED local e persiste sem Meta ou Hermes",async({page})=>{test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar resposta local.");await login(page,account("ATTENDANT"));
    ...
    expect(refreshGets).toHaveLength(5);expect(refreshGets.filter(path=>path==="/v1/inbox/availability")).toHaveLength(1);expect(refreshGets.filter(path=>path==="/v1/inbox/capacity-alert")).toHaveLength(0);expect(refreshGets.filter(path=>/^\/v1\/inbox\/conversations\/[^/]+(?:\/messages)?$/u.test(path))).toHaveLength(2);expect(mutations.filter(value=>!value.endsWith("/claim"))).toEqual([]);
  ...
  test("duas sessões convergem após hidden e offline sem clique ou POST observador",async({browser})=>{test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar convergência multi-sessão local.");const actorContext=await browser.newContext(),observerContext=await browser.newContext(),actor=await actorContext.newPage(),observer=await observerContext.newPage();const observerPosts:string[]=[],observerGets:string[]=[],externalHosts:string[]=[];let observe=false;
    observer.on("request",request=>{const url=new URL(request.url());if(url.hostname!=="zap-pronto.127.0.0.1.nip.io")externalHosts.push(url.host);if(observe&&request.method()==="GET"&&url.pathname.startsWith("/v1/inbox/"))observerGets.push(url.pathname);if(observe&&url.pathname.startsWith("/v1/")&&["POST","PUT","PATCH","DELETE"].includes(request.method()))observerPosts.push(`${request.method()} ${url.pathname}`)});
    try{await login(actor,account("ATTENDANT"));await actor.getByRole("button",{name:"Contato · NORMAL"}).click();await actor.getByRole("button",{name:"Assumir atendimento"}).click();await expect(actor.getByRole("button",{name:"Contato · Em atendimento"})).toBeVisible();await login(observer,account("ATTENDANT"));await observer.getByRole("button",{name:"Contato · Em atendimento"}).click();await expect(observer.getByRole("textbox",{name:"Mensagem"})).toBeVisible();
      await observer.evaluate(()=>{const state=window as Window&{__e2eHidden?:boolean};state.__e2eHidden=true;Object.defineProperty(document,"visibilityState",{configurable:true,get:()=>state.__e2eHidden?"hidden":"visible"});document.dispatchEvent(new Event("visibilitychange"))});await expect.poll(()=>observer.evaluate(()=>document.visibilityState)).toBe("hidden");await observerContext.setOffline(true);observe=true;
      const actorEditor=actor.getByRole("textbox",{name:"Mensagem"});await actorEditor.fill("Convergência externa sintética");await actor.getByRole("button",{name:"Enviar"}).click();await expect(actor.getByText("Convergência externa sintética")).toHaveCount(1);
      await observerContext.setOffline(false);expect(observerGets).toHaveLength(0);await observer.evaluate(()=>{(window as Window&{__e2eHidden?:boolean}).__e2eHidden=false;document.dispatchEvent(new Event("visibilitychange"))});await expect.poll(()=>observer.evaluate(()=>document.visibilityState)).toBe("visible");await expect.poll(()=>observerGets.length,{timeout:10_000}).toBeGreaterThan(0);await expect(observer.getByText("Convergência externa sintética")).toHaveCount(1,{timeout:10_000});expect(observerPosts).toEqual([]);expect(externalHosts).toEqual([]);
    }finally{await actorContext.close();await observerContext.close()}
  });
  ...
  test("transfere atendimento entre dois atendentes da mesma unidade",async({browser})=>{test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar transferência local.");
    ...
    const externalHosts:string[]=[];const transferPosts:string[]=[];const targetMutations:string[]=[];const targetReads:string[]=[];let observeTarget=false;
    for(const page of[ownerPage,targetPage])page.on("request",request=>{const url=new URL(request.url());if(url.hostname!=="zap-pronto.127.0.0.1.nip.io")externalHosts.push(url.host);if(request.method()==="POST"&&url.pathname.endsWith("/transfer"))transferPosts.push(url.pathname)});
    targetPage.on("request",request=>{if(!observeTarget)return;const url=new URL(request.url());if(!url.pathname.startsWith("/v1/"))return;if(request.method()==="GET")targetReads.push(url.pathname);else targetMutations.push(`${request.method()} ${url.pathname}`)});
    try{await login(ownerPage,account("ATTENDANT"));await login(targetPage,account("ATTENDANT_TWO"));observeTarget=true;await ownerPage.getByRole("button",{name:"Contato · NORMAL"}).click();await ownerPage.getByRole("button",{name:"Assumir atendimento"}).click();
      await expect(targetPage.getByRole("button",{name:"Contato · Sob supervisão"})).toBeVisible({timeout:40_000});
      ...
      const targetActive=targetPage.getByRole("button",{name:"Contato · Em atendimento"});await expect(targetActive).toBeVisible({timeout:40_000});await expect(targetPage.getByRole("button",{name:"Contato · Sob supervisão"})).toHaveCount(0);await targetActive.click();await expect(targetPage.getByText("Estado: HUMAN_ACTIVE")).toBeVisible();
      expect(targetReads.filter(path=>path==="/v1/inbox/active").length).toBeGreaterThan(0);expect(targetReads.filter(path=>path==="/v1/inbox/supervised").length).toBeGreaterThan(0);expect(targetMutations).toEqual([]);expect(externalHosts).toEqual([]);await expect(targetPage.getByText("Enviado",{exact:true})).toHaveCount(0);
  ...
  test("administrador revoga e reativa vínculo, força OFFLINE e impede claim",async({browser})=>{test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para o lifecycle de membership local.");const identifier=process.env.E2E_ATTENDANT_TWO_ADMIN_LIST_MATCH;if(!identifier)throw new Error("E2E_ATTENDANT_TWO_ADMIN_LIST_MATCH_REQUIRED");const admin=await browser.newPage(),attendant=await browser.newPage();await login(admin,account("ADMIN"));await openModule(admin,"Acessos");
    const openMembership=async()=>{const row=await userRow(admin,identifier);return row.getByRole("list",{name:"Vínculos de Atendente Local 2"})};
    try{await login(attendant,account("ATTENDANT_TWO"));await attendant.getByRole("button",{name:"Alterar disponibilidade"}).click();await attendant.getByLabel("Status da disponibilidade").selectOption("AVAILABLE");const available=attendant.waitForResponse(response=>response.request().method()==="POST"&&new URL(response.url()).pathname.endsWith("/availability"));await attendant.getByRole("button",{name:"Confirmar alteração"}).click();expect((await available).status()).toBe(200);
      const membership=await openMembership();await membership.getByRole("button",{name:"Revogar vínculo"}).click();await admin.getByLabel("Motivo").fill("Homologação lifecycle unitário");const revoked=admin.waitForResponse(response=>new URL(response.url()).pathname.endsWith("/lifecycle")&&response.request().method()==="POST");await admin.getByRole("button",{name:"Confirmar revogar vínculo"}).click();expect((await revoked).status()).toBe(200);await expect((await openMembership()).getByText(/Revogado/)).toBeVisible();
      const reactivate=(await openMembership()).getByRole("button",{name:"Reativar vínculo"});await reactivate.click();await admin.getByLabel("Motivo").fill("Reativação controlada da homologação");const restored=admin.waitForResponse(response=>new URL(response.url()).pathname.endsWith("/lifecycle")&&response.request().method()==="POST");await admin.getByRole("button",{name:"Confirmar reativar vínculo"}).click();expect((await restored).status()).toBe(200);await expect((await openMembership()).getByText(/Ativo/)).toBeVisible();
      await attendant.reload();await expect(attendant.getByText(/Offline/)).toBeVisible();await attendant.getByRole("button",{name:"Contato · NORMAL"}).click();const denied=attendant.waitForResponse(response=>response.request().method()==="POST"&&new URL(response.url()).pathname.endsWith("/claim"));await attendant.getByRole("button",{name:"Assumir atendimento"}).click();expect((await denied).status()).toBe(409);await expect(attendant.getByRole("alert")).toContainText("Outro atendimento venceu esta disputa");
    }finally{await admin.close();await attendant.close()}
  });
  test("mudança de fuso invalida escalas antigas e mantém enforcement fail-closed",async({browser})=>{
    test.skip(!enabled,"Defina E2E_OIDC_ENABLED=true para homologar invalidação de escala por fuso.");const manager=await browser.newPage(),attendant=await browser.newPage();const externalHosts:string[]=[];
    for(const page of[manager,attendant])page.on("request",request=>{const url=new URL(request.url());if(url.hostname!=="zap-pronto.127.0.0.1.nip.io")externalHosts.push(url.host)});
    try{await login(manager,account("MANAGER"));await openModule(manager,"Escalas");await expect(manager.getByText(/^\d+ de \d+ integrantes operacionais possuem escala vigente; \d+ pendentes\.$/u)).toBeVisible();await manager.getByLabel("Fuso IANA").fill("America/Sao_Paulo");await manager.getByRole("button",{name:"Salvar fuso operacional"}).click();const changed=manager.waitForResponse(response=>response.request().method()==="POST"&&/\/operational-timezone$/u.test(new URL(response.url()).pathname));await manager.getByRole("button",{name:"Confirmar alteração"}).click();expect((await changed).status()).toBe(200);await expect(manager.getByText(/Fuso vigente: America\/Sao_Paulo/)).toBeVisible();await manager.getByRole("button",{name:"Atualizar política"}).click();await expect(manager.getByText(/^\d+ de \d+ integrantes operacionais possuem escala vigente; \d+ pendentes\.$/u)).toBeVisible();
      await login(attendant,account("ATTENDANT"));await attendant.getByRole("button",{name:"Contato · NORMAL"}).click();const denied=attendant.waitForResponse(response=>response.request().method()==="POST"&&new URL(response.url()).pathname.endsWith("/claim"));await attendant.getByRole("button",{name:"Assumir atendimento"}).click();expect((await denied).status()).toBe(409);await expect(attendant.getByRole("alert")).toContainText("fora do turno configurado");await expect(attendant.getByRole("button",{name:"Contato · NORMAL"})).toBeVisible();expect(externalHosts).toEqual([]);
    }finally{await manager.close();await attendant.close()}
  });
